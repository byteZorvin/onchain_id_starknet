...
fn test_should_panic_when_update_implementation_when_class_hash_zero() {
    let mut implementation_authority = deploy();
    cheat_caller_address(implementation_authority.contract_address, OWNER_ADDRESS(), CheatSpan::TargetCalls(1));
...
fn test_should_panic_when_upgrade_identity_when_implementations_are_identical() {
    ...
    cheat_caller_address(setup.alice_identity.contract_address, setup.accounts.alice_account.contract_address, CheatSpan::TargetCalls(1));
    setup.alice_identity.execute(setup.implementation_authority.contract_address, selector!("upgrade_identity"), [].span());
}
...
fn test_should_upgrade_identity() {
    ...
    let claim_issuer_class_hash = starknet::syscalls::get_class_hash_at_syscall(setup.claim_issuer.contract_address).unwrap();

    cheat_caller_address(setup.implementation_authority.contract_address, setup.accounts.owner_account.contract_address, CheatSpan::TargetCalls(1));
    setup.implementation_authority.update_implementation(claim_issuer_class_hash);

    cheat_caller_address(setup.alice_identity.contract_address, setup.accounts.alice_account.contract_address, CheatSpan::TargetCalls(1));
    setup.alice_identity.execute(setup.implementation_authority.contract_address, selector!("upgrade_identity"), [].span());

    let alice_identity_class_hash = starknet::syscalls::get_class_hash_at_syscall(setup.alice_identity.contract_address).unwrap();
